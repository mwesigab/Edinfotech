                ...
                <div class="row">
                    <div class="col-md-3">
                        <div class="input-group">
                            <input type="date" value="{!! $_GET['fsdate'] ?? '' !!}" id="fsdate" class="text-center form-control" name="fsdate" placeholder="Start Date">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped mb-none" id="datatable-details">
                    ...
                    @foreach($lists as $item)
                        <tr>
                            <td class="cu-p" data-toggle="modal" href="#description{{{ $item->id ?? 0 }}}">{{{ $item->title }}}</td>
                            <td class="text-center" width="150">{{{ date('d F Y / H:i',$item->create_at) }}}</td>
                            <td class="text-center" title="{{{ $item->requester->username ?? '' }}}">{{{ $item->requester->name ?? '' }}}</td>
                            <td class="text-center" title="{{{ $item->user->username ?? '' }}}">{{{ $item->user->name ?? '' }}}</td>
                            <td class="text-center">{{{ $item->fans_count ?? 0 }}}</td>
                            <td class="text-center">{{{ $item->category->title ?? '' }}}</td>
                            <td class="text-center"><a href="/product/{{{ $item->content->id ?? 0 }}}" target="_blank">{{{ $item->content->title ?? '' }}}</a></td>
...
@section('modals')
    @foreach($lists as $item)
        <div class="modal fade" role="dialog" id="description{{{ $item->id ?? 0 }}}">
