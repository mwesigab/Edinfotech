...
@section('script')
    <script>
        $(function() {
            pagination('.blog-section',{{{ $setting['site']['blog_post_count'] ?? 4 }}},0);
        $('.pagi').pagination({
            items: {!! count($posts) !!},
            itemsOnPage:{{{ $setting['site']['blog_post_count'] ?? 4 }}},
            cssStyle: 'light-theme',
            prevText: 'Pre.',
            nextText:'Next',
            onPageClick:function(pageNumber, event) {
            pagination('.blog-section',{{{ $setting['site']['blog_post_count'] ?? 4 }}},pageNumber-1);
        }
        });
        });
    </script>
